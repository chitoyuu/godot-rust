...
godot_test!(
    test_array_debug {
        ...
        let panic_get = catch_unwind(|| { arr.get(3); });
        let panic_get_ref = catch_unwind(|| { unsafe { arr.get_ref(3) }; });
        let panic_get_mut_ref = catch_unwind(|| { unsafe { arr.get_mut_ref(3) }; });

        assert!(panic_get.is_err(), "get() out of bounds causes panic");
        assert!(panic_get_ref.is_err(), "get_mut() out of bounds causes panic");
        assert!(panic_get_mut_ref.is_err(), "get_mut_ref() out of bounds causes panic");
    }
);
